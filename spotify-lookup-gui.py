...
artist_name = simpledialog.askstring('Enter artist name', "Enter the artist name: ")
artist_data = search_artists(artist_name, access_token)
#Create a GUI:
root = Tk()
root.title('Spotify lookup')
root.geometry('500x500')
head=Label(root, text='Found the following artists:', font=('Arial', 20))
head.pack(pady=20)
mainlist_frame = Frame(root, highlightthickness=2, highlightbackground='black')
mainlist_frame.pack()
mainlist = Listbox(mainlist_frame, width=50, height=450, font='Arial')
mainlist.pack(side=LEFT)
scrollbar = Scrollbar(mainlist_frame, orient=VERTICAL)
scrollbar.pack(side=RIGHT)
for idx , (name, id) in enumerate(artist_data, start=1):
	mainlist.insert(idx, name)
