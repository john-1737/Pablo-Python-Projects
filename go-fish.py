    ...
    def draw(self):
        if self.card in ('J', 'Q', 'K', 'joker'):
            for i in ((25,75), (75,75)):
                self.canvas.create_text(i[0], i[1], text=self.suit)
            self.canvas.create_text(50, 75, text={'K': '🤴', 'Q': '👸', 'J':'💂', 'joker':'🤹'}[self.card], font=('Arial', 15))
        else:
            for i in card_poss[self.card]:
                self.canvas.create_text(i[0], i[1], text=self.suit, font=('Arial', 15))
        self.canvas.create_text(2, 2, text=f'{self.card}\n{self.suit}', anchor='nw', font=('Arial', 10))
        self.canvas.create_text(98, 148, text=f'{self.suit}\n{self.card}', anchor='se', font=('Arial', 10))
