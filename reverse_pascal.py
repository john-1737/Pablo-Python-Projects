    ...
    for i in range(2, 11):
        for j in range(1, 11):
            if j == 1:
                ...
                nums[i, j] = nums[i-1, j] + nums[i-1, j - 1] + nums[i-1, j + 1]
    for i in nums:
        nums[i] = str(nums[i]).ljust(7)
    t.delete('1.0', 'end')
    t.insert('1.0', '''#{}#{}#{}#{}#{}#{}#{}#{}#{}#{}#
...
Button(root, text='Calculate Triangle', command=calculate_triangle).pack()
Button(root, text='Calculate Square', command=calculate_square).pack()
