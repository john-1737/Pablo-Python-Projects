...
def main():
    global win, wininp, shell_entry, entlbl, terminal_entry, output, inp, shell, editor, master, folder, files, listb, selfile, n, b1, b2, style
    master = tk.Tk()
    master.title('Editor')
    ...
    for i in style.theme_names():
        theme_menu.add_command(label=i, command=lambda: style.theme_use(i))
    menu_settings.add_cascade(menu=theme_menu, label='Themes')
    menubar.add_cascade(menu=menu_settings, label='Settings')
    n = ttk.Notebook(master)
    editor = ttk.Frame(master)
    shell = ttk.Frame(master)
    n.add(editor, text='Editor')
    n.add(shell, text='Python Shell')
    n.pack()
    ttk.Button(editor, text='+ Create', command=lambda: inp.delete(1.0, 'end')).grid(column=0, row=0)
    ttk.Button(editor, text='⇧ Open', command=lambda:id.ask_open_file(inp)).grid(column=1, row=0)
    ttk.Button(editor, text='⇩ Save', command=lambda:id.save_file(inp)).grid(column=2, row=0)
    ttk.Button(editor, text='⏵ Run', command=launch).grid(column=3, row=0)
    ttk.Button(editor, text='⏵ Run File', command=launch_file).grid(column=4, row=0)
    ...
    entlbl = ttk.Label(master, text='Enter commands for terminal')
    entlbl.pack()
    terminal_var = tk.StringVar()
    terminal_entry = ttk.Entry(master, textvariable=terminal_var, width=60)
    terminal_entry.pack()
    ...
    file_manager = ttk.Frame(master)
    file_manager_labels = []
    n.add(file_manager, text='File Manager')
    folder = tk.StringVar(value=os.getcwd())
    l = ttk.Label(file_manager, textvariable=folder)
    l.grid(column=0, row=0, columnspan=2, sticky='w')
    file_manager_labels.append(l)
    ttk.Button(file_manager, text='Change Directory', command=setfolder).grid(column=0, row=1)
    ...
    l = ttk.Label(file_manager, textvariable=selfile)
    l.grid(column=1, row=2)
    file_manager_labels.append(l)
    listb.bind('<<ListboxSelect>>', show_buttons)
    b1 = ttk.Button(file_manager, text='Open in editor', command=lambda: open_editor(os.listdir(folder.get())[listb.curselection()[0]]))
    b2 = ttk.Button(file_manager, text='Open in EasyEdit', command=lambda: open_editdialog(os.listdir(folder.get())[listb.curselection()[0]], win))
    b1.grid(column=1, row=3, sticky='n')
    b2.grid(column=1, row=4, sticky='n')
    b1.grid_remove()
    b2.grid_remove()
    ttk.Button(file_manager, text='Open in default application', command=lambda : os.system('open ' + os.listdir(folder.get())[listb.curselection()[0]])).grid(column=1, row=5)
