        ...
        for event in pg.event.get():
            if event.type == QUIT:
                ...
                if event.key == K_SPACE:
                    ...
                    sys.exit()
        screen.fill((0,0,0))
        render_text('Welcome to 2048!', (0, 0), font=smallfont)
        render_text('Slide all the tiles on the board in one of four', (0, 25), font=smallfont)
        render_text('directions with the arrow keys.', (0, 50), font=smallfont)
        render_text('Tiles with like numbers will combine into', (0, 75), font=smallfont)
        render_text('larger-numbered tiles.', (0, 100), font=smallfont)
        render_text('A new 2 tile is added to the board on each move.', (0, 125), font=smallfont)
        render_text('You win if you can create a 2048 tile.', (0, 150), font=smallfont)
        render_text('You lose if the board fills up the tiles before then.', (0, 175), font=smallfont)
        render_text('Press SPACE to start!', (0, 200), font=smallfont)
        pg.display.update()
    ...
    while True:
        ...
        while game:  # Main game loop.
            ...
            if isFull(gameBoard):
                ...
                render_text('Score: '+ str(getScore(gameBoard)), (0, 400))
                render_text('Press BACKSPACE to reset.', (0, 450), font=smallfont)
            pg.display.update()
        ...
        while endgame:
            ...
            screen.fill((0, 0, 0))
            drawBoard(gameBoard)
            render_text('Game Over!', (0, 400))
            render_text(f'Your score was {getScore(gameBoard)}.', (0, 450), font=smallfont)
            render_text('Press SPACE to play again.', (0, 475), font=smallfont)
            pg.display.update()
...
def drawBoard(board):
    ...
    for y in range(4):
        for x in range(4):
            ...
            pg.draw.rect(screen, (100, 100, 100), pg.Rect(x*100, y*100, 100, 100), 10)
            if tile != BLANK:
                num = 2
                color = 0
                while num != tile:
                    color += 1
                    num *= 2
                pg.draw.rect(screen, colors[color%6], pg.Rect(x*100, y*100, 100, 100))
                render_text(str(tile), ((x*100)+50, (y*100)+50), True, (0, 0, 0))
