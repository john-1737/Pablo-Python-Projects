...
class Tower_of_hanoi_game:
    def __init__(self, root, num_disks):
        ...
        self.canvas = tk.Canvas(self.mainframe, width=400, height=100)
        self.canvas.grid(column=0, row=3, columnspan=2, pady=20)
        tk.Label(self.mainframe, text='Select a tower to move a disk from.').grid(column=0, row=0, sticky='w', pady=20)
        tk.Label(self.mainframe, text='Select a tower to move a disk to.').grid(column=0, row=1, sticky='w', pady=20)
        self.from_tower = tk.IntVar()
        self.to_tower = tk.IntVar()
        self.from_tower_entry = tk.Entry(self.mainframe, width=7, textvariable=self.from_tower).grid(column=1, row=0)
        self.to_tower_entry = tk.Entry(self.mainframe, width=7, textvariable=self.to_tower).grid(column=1, row=1)
        self.submit_button = tk.Button(self.mainframe, text='Submit', command=self.submit_move).grid(column=0, row=2)
        self.towers = [[4,3,2,1], [], []]
        return None
