...
def draw_card(card, suit, canvas):
    ...
    if card in ('J', 'Q', 'K', 'joker'):
        for i in ((25, 75), (75, 75)):
            canvas.create_image(i[0], i[1], image=suit)
        canvas.create_image(50, 75, image={'K': king, 'Q': queen, 'J': jack}[card])
    else:
        for i in card_poss[card]:
            canvas.create_image(i[0], i[1], image=suit)
    canvas.create_text(2, 2, text=card, anchor='nw', font=('Arial', 15))
    canvas.create_text(98, 148, text=card, anchor='se', font=('Arial', 15))
...
def main():
    ...
    club, diamond, heart, jack, king, queen, spade
    card_canvases = []
    root = Tk()
