...
{'question': "A taxi driver is going the wrong way down a one-way street.\nShe passes ten cops but doesn't get a ticket. Why not?",
 ...
 {'question': "If you're in the middle of a square house facing the west side with the south side\nto your left and the north side to your right, which side of the house are you next to?",
 ...
 {'question': "What five-letter word becomes shorter by adding two letters?",
  'answer': "Short.",
  'accept': ['short']},
 {'question': "Gwen's mother has five daughters. Four are named Haha, Hehe, Hihi, and Hoho.\nWhat's the fifth daughter's name?",
  'answer': "Gwen.",
  'accept': ['gwen']},
 ...
 {'question': "Where was the United States Declaration of Independence signed?",
  ...
  'accept': ['bottom']},
 {'question': "A person puts two walnuts in their pocket but only has one thing in their pocket\nfive minutes later. What is it?",
  'answer': "A hole.",
  'accept': ['hole']},
 ...
 {'question': "If you drop a raw egg on a concrete floor, will it crack?",
  ...
  'accept': ['no']},
 {'question': "If it takes ten people ten hours to build a fence,\nhow many hours does it take five people to build it?",
 ...
 {'question': "An electric train leaves the windy city of Chicago at 4pm on a Monday heading south\nat 100 kilometers per hour. Which way does the smoke blow from the smokestack?",
 ...
 {'question': "A doctor gives you three pills with instructions to take one every half-hour.\nHow long will the pills last?",
  'answer': "One hour.",
  'accept': ['1', 'one']},
...
def next_question():
    ...
    if questionNumber == len(QUESTIONS):
        ...
        questionNumber = -1
        score = 0
        question_var.set('That\'s all the questions. Thanks for playing!')
        correct_var.set('Press Restart to play again.')
        b.config(text='Restart', command=next_question)
        return
    question_num.set('Question: '+ str(questionNumber + 1))
    score_var.set(f'Score: {score}/{len(QUESTIONS)}')
    question_var.set(QUESTIONS[questionNumber]['question'])
    answer_var.set('')
    b.config(text='Submit', command=check)

def check():
    global score
    aentry.grid_remove()
    alabel.grid()
    qa = QUESTIONS[questionNumber]
    correct = False
    for acceptanceWord in qa['accept']:
        if acceptanceWord in answer_var.get():
            correct = True
    if correct:
        text = random.choice(CORRECT_TEXT)
        question_var.set(text)
        correct_var.set(qa['answer'])
        score += 1
    else:
        text = random.choice(INCORRECT_TEXT)
        question_var.set(text)
        correct_var.set('The answer is: '+ qa['answer'])
    score_var.set(f'Score: {score}/{len(QUESTIONS)}')
    b.config(text='Next question', command=next_question)

root = Tk()
root.title('Trick Questions')
#root.geometry('550x100')
win = Frame(root)
win.grid(sticky='nwes')
question_num = StringVar(value='Can you figure out the answers to these trick questions?')
score_var = StringVar(value='Questions and original program by Al Sweigart.')
question_var = StringVar()
answer_var = StringVar()
correct_var = StringVar()
Label(win, textvariable=question_num).grid(column=0, row=0)
Label(win, textvariable=score_var).grid(column=0, row=1)
font_dict = nametofont('TkDefaultFont').actual()
font_dict['weight'] = 'bold'
bold_font = Font(**font_dict)
Label(win, textvariable=question_var, font=bold_font).grid(column=0, row=2)
aentry = Entry(win, textvariable=answer_var)
aentry.grid(column=0, row=3); aentry.grid_remove()
alabel = Label(win, textvariable=correct_var)
alabel.grid(column=0, row=3); alabel.grid_remove()
questionNumber = -1
score = 0
b = Button(win, text='Start', default='active', command=next_question)
